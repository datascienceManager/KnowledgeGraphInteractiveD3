...
<body>
    ...
    <script>
        ...
        function hideTooltip() {{
            ...
            // Remove selection when hiding tooltip
            d3.selectAll(".node").classed("selected", false);
            selectedNode = null;
        }}
    ...
    print(f"🎨 Features:")
    print(f"   - Black background + Custom TOD logo with glow")
    print(f"   - Node sizes based on viewer count")
    print(f"   - Glowing buttons (no thick borders)")
    print(f"   - Click nodes for glow effect + persistent tooltip")
    print(f"📊 Total nodes: {len(data['nodes'])}")
    print(f"🔗 Total links: {len(data['links'])}")
    print(f"🎬 Total clusters: {len(set(node['cluster'] for node in data['nodes']))}")
    print(f"\n💡 Open '{output_filename}' in your web browser!")
